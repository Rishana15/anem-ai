...
class RegisterFragment : Fragment() {
    ...
    private fun showAlertDialogSuccess(title: String, message: String) {
        ...
        AlertDialog.Builder(this.requireContext())
            .setTitle(title)
            .setMessage(message)
            .setCancelable(false) // Prevents user from clicking outside and getting stuck
            .setPositiveButton("Login now") { dialog, _ ->
                dialog.dismiss()
                // Navigate ONLY here to avoid the "Destination not found" crash
    ...
    private fun showAlertDialogFailure(title: String, message: String) {
        ...
        // Explicitly use the Material Alert Dialog theme to fix Resource ID #0x0
        AlertDialog.Builder(context, com.google.android.material.R.style.ThemeOverlay_MaterialComponents_MaterialAlertDialog)
            .setTitle(title)
            .setMessage(message)
